...
void	my_mlx_pixel_put(t_img *img, t_cam cam, int x, int y)
{
    ...
    ray = make_ray(cam.origin, sub_vec(add_vec(add_vec(cam.left_bottom_corner, \
                                                ...
                                                cam.origin));
    if (is_hit_sphere(img->sp, ray, &rec, 999999))
    {
        tmp = unit_vec(rec.normal);
...
int main(void)
{
    ...
    mlx->win = mlx_new_window(mlx->mlx_ptr, WIN_WIDTH, WIN_HEIGHT, "Sphere Gradation");
    ...
    img.sp = malloc(sizeof(t_sphere) * 2);
    img.sp[0] = make_sphere(make_vec(0, 0, -1), 0.5);
    img.sp[1] = make_sphere(make_vec(0, -100.5, -1), 100);
    cam = make_cam(make_vec(0, 0, 0), 2.0 * 16.0 / 9.0, 2.0, 1.0);
    count_h = IMG_HEIGHT;
    while (--count_h >= 0)
    {
        count_w = -1;
